...
function imageInfo(post, id) {
  ...
  if (/^https?:\/\//i.test(raw)) {
    ...
    return { url: `${APP_URL}/api/postImage?id=${encodeURIComponent(id)}`, type };
  }
  return null;
}
  ...
  try {
    if (id) {
      ...
      if (snap.exists) {
        ...
        const image = imageInfo(post, id) || youtubeImage(post.youtube);
        data = { title: String(post.title || "Yeni Ozanlar").trim(), description: desc(post.text), image: image?.url || null, imageType: image?.type || null, canonical };
      }
    }
